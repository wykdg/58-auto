...
def publish(account,info):
    check_states=info.check_states
    xiaoqu=info.xiaoqu
    huxingshi=info.huxingshi
    huxingting=info.huxingting
    huxingwei=info.huxingwei
    area=info.area
    ceng=info.ceng
    zongceng=info.zongceng
    toward=info.toward
    zhuangxiu=info.zhuangxiq
    leixing=info.leixing
    zujin=info.zujin
    yajin=info.yajin
    biaoti=info.biaoti
    miaosu=info.miaosu
    lianxiren=info.lianxiren
    phone=info.phone
    ...
    browser.find_element_by_id('huxingshi').send_keys(huxingshi)
    ...
    browser.find_element_by_id('Floor').send_keys(ceng)
    browser.find_element_by_id('zonglouceng').send_keys(zongceng)
    browser.find_element_by_id('MinPrice').send_keys(zujin)
    ...
    browser.find_element_by_xpath('/html/body').send_keys(miaosu)
    browser.switch_to.parent_frame()
    browser.find_element_by_id('goblianxiren').send_keys(lianxiren)
    ...
    for selector,value in zip(selects,[toward,zhuangxiu,leixing,yajin,localArea,localDiduan]):
